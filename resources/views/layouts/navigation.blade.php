                ...
                <div class="hidden sm:ms-6 sm:flex sm:flex-1">
                    ...
                    @if (auth()->user()->role === 'admin')
                        ...
                        <div class="relative group flex items-center h-full">
                            ...
                            <div class="absolute left-0 top-full z-50 mt-0 w-64 rounded-md border border-gray-200 bg-white shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                ...
                                <a href="{{ route('schedules.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-blue-50">ตารางเรียน</a>
                            </div>
                        </div>
                    ...
                    <x-nav-link class="text-white hover:text-blue-200" :href="route('messages.index')" :active="request()->routeIs('messages.index')">
                        ...
                        @if(($unreadMessageCount ?? 0) > 0)
                            <span class="ml-1 inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-xs font-bold text-white bg-red-500 rounded-full leading-none">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('messages.index')" :active="request()->routeIs('messages.index')">
                ข้อความ
                @if(($unreadMessageCount ?? 0) > 0)
                    <span class="ml-2 inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-xs font-bold text-white bg-red-500 rounded-full leading-none">
                        {{ $unreadMessageCount > 99 ? '99+' : $unreadMessageCount }}
                    </span>
                @endif
            </x-responsive-nav-link>
