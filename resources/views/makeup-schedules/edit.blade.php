<x-app-layout>
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-lg p-8 mb-8 flex items-center justify-between gap-4">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800">แก้ไขตารางชดเชย</h1>
                    <p class="text-gray-600 mt-2">ปรับปรุงวัน เวลา และรายละเอียดการเรียนชดเชย</p>
                </div>
                <a href="{{ route(auth()->user()->role === 'teacher' ? 'makeup-schedules.teacher-schedule' : 'makeup-schedules.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-5 py-2 rounded-lg font-semibold transition">
                    ← ย้อนกลับ
                </a>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-8">
                ...
                <form action="{{ route('makeup-schedules.update', $makeupSchedule) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="class_id" name="class_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('class_id') border-red-500 @enderror" required>
                                ...
                                @foreach($classes as $class)
                                    <option value="{{ $class->id }}" {{ old('class_id', $makeupSchedule->class_id) == $class->id ? 'selected' : '' }}>{{ $class->class_name }} ({{ $class->class_code }})</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="subject_id" name="subject_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('subject_id') border-red-500 @enderror" required>
                                ...
                                @foreach($subjects as $subject)
                                    <option value="{{ $subject->id }}" {{ old('subject_id', $makeupSchedule->subject_id) == $subject->id ? 'selected' : '' }}>{{ $subject->subject_code }} - {{ $subject->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="date" id="makeup_date" name="makeup_date" value="{{ old('makeup_date', optional($makeupSchedule->makeup_date)->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('makeup_date') border-red-500 @enderror" required>
                        ...
                        <div>
                            ...
                            <select id="semester_id" name="semester_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('semester_id') border-red-500 @enderror" required>
                                ...
                                @foreach($semesters as $semester)
                                    <option value="{{ $semester->id }}" {{ old('semester_id', $makeupSchedule->semester_id) == $semester->id ? 'selected' : '' }}>{{ $semester->name }} (พ.ศ. {{ $semester->year }})</option>
                                @endforeach
                            </select>
                            ...
                            <select id="start_time" name="start_time" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('start_time') border-red-500 @enderror" required>
                                ...
                                @for ($h = 7; $h <= 22; $h++)
                                    @foreach (['00', '30'] as $m)
                                        ...
                                        <option value="{{ $t }}" {{ old('start_time', substr((string) $makeupSchedule->start_time, 0, 5)) == $t ? 'selected' : '' }}>{{ $t }} น.</option>
                                    @endforeach
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select id="end_time" name="end_time" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('end_time') border-red-500 @enderror" required>
                                ...
                                @for ($h = 7; $h <= 22; $h++)
                                    @foreach (['00', '30'] as $m)
                                        @php $t = str_pad($h, 2, '0', STR_PAD_LEFT) . ':' . $m; @endphp
                                        <option value="{{ $t }}" {{ old('end_time', substr((string) $makeupSchedule->end_time, 0, 5)) == $t ? 'selected' : '' }}>{{ $t }} น.</option>
                                    @endforeach
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select id="room" name="room" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('room') border-red-500 @enderror" required>
                                ...
                                @foreach($rooms as $room)
                                    <option value="{{ $room }}" {{ old('room', $makeupSchedule->room) === $room ? 'selected' : '' }}>{{ $room }}</option>
                                @endforeach
                            </select>
                            ...
                            @else
                                <select id="teacher_id" name="teacher_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('teacher_id') border-red-500 @enderror" required>
                                    ...
                                    @foreach($teachers as $teacher)
                                        <option value="{{ $teacher->id }}" {{ old('teacher_id', $makeupSchedule->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }}</option>
                                    @endforeach
                                </select>
                            @endif
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('notes') border-red-500 @enderror">{{ old('notes', $makeupSchedule->notes) }}</textarea>
                        @error('notes') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="flex gap-3">
                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-semibold transition">บันทึกการแก้ไข</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const currentMakeupScheduleId = '{{ $makeupSchedule->id }}';
            ...
            const refreshRooms = async () => {
                const params = new URLSearchParams({
                    ...
                    exclude_makeup_schedule_id: currentMakeupScheduleId,
                });
